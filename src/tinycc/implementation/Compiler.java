...
public class Compiler {
	...
	ASTFactoryImplementation astFactory;
	...
	public Compiler(final Diagnostic diagnostic) {
		...
		this.astFactory = new ASTFactoryImplementation();
	}
	...
	public void checkSemantics() {
		...
		this.astFactory.getTranslationUnit().stream().forEach(decl -> decl.checkCode(diagnostic, globalScope));
	}
	...
	public void generateCode(final MipsAsmGen out) {
		CompilationScope offs = new CompilationScope();
		this.astFactory.getTranslationUnit().stream().forEach(decl -> decl.generateCode(out, offs));
	}
	...
	public Formula genVerificationConditions() {
		long funcDef = this.astFactory.getTranslationUnit().stream()
			.filter(decl -> decl instanceof FunctionDefinition).count();
		if (funcDef > 1) {
			throw new IllegalStateException("Only one function definition is allowed");
		}
		ExternalDeclaration function;
		function = this.astFactory.getTranslationUnit().stream()
								  .filter(decl -> decl instanceof FunctionDefinition)
							      .findFirst().get();
		return function.genFormula();
	}
}
